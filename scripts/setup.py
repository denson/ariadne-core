...
def show_connection(url):
    ...
    print("  which reads a Bearer JWT from the OS keyring and emits")
    print("    Authorization: Bearer <jwt>")
    print("  for Claude Code. To populate the keyring:")
    print()
    print("    1. Run: ariadne login --host <host>")
    print("    2. Browser opens; authenticate against Auth0")
    print("    3. Tokens cached in OS keyring (Windows Credential Manager,")
    print("       macOS Keychain, or Linux Secret Service)")
    print()
    print("  See docs/smoke/xft5c_smoke.md for the full operator walkthrough.")
    print("  ARIADNE_ACCESS_TOKEN env var remains as a CI/automation escape")
    print("  hatch (bypasses the keyring); never paste a JWT into .env on disk.")
    print()
