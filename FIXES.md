...
**Target state:** The `instructions` string in the `FastMCP()` constructor reflects the full tool set and correct format count. It should mention `ingest` and `list_collections` and stop telling agents to use the REST API for batch operations.
...
- Mention the full tool set: `convert_document`, `search`, `get_document`, `list_documents`, `list_collections`, `ingest`
...
**How to test:**
- Start the MCP server. Connect a client. Verify the instructions block appears in the server capabilities and mentions the full tool set.
