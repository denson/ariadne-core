# Task: Rewrite ariadne-document-intelligence SKILL.md
...
Complete rewrite of `ariadne-core/skills/ariadne-document-intelligence/SKILL.md`. The current version (577 lines) is built around MCP tools (`convert_document`, `search`, `ingest` as MCP calls, upload+convert two-step pattern, `bulk_ingest.py`, `.mcp.json` credentials, Open Brain bridge, `claude-cowork` references). All of that is gone. The new version is built around the `ariadne-core-client` Python package and the CLI.

**Replace the entire file** with the content below. Do not try to surgically edit — the structure changed too much.
...
## What changed (summary for Dave)

1. **MCP tools → client package methods**: `convert_document` → `client.ingest_url/file/bytes`, `search` → `client.search()`, etc.
2. **Two-step upload+convert → one-call client methods**: No more `POST /api/upload` then `convert_document`
3. **`bulk_ingest.py` → CLI `ariadne ingest`**: Script references removed entirely
4. **Connection check**: `list_collections` MCP call → `client.health()` with proper client setup
5. **Open Brain / OB1 section**: Removed entirely (dead integration)
6. **`claude-cowork` → `claude-code`**: Updated agent_type examples
7. **`.mcp.json` credentials → env vars**: Client resolves from `ARIADNE_URL`/`ARIADNE_API_KEY`
8. **`VISION_API_KEY` → `ARIADNE_IMAGE_ENRICHMENT_API_KEY`**: Updated env var name
9. **Added**: `update_document`, `delete_document`, `restore_document`, soft-delete section
10. **Added**: `source` convenience parameter documentation
11. **Added**: Dataclass return types and exception handling notes
12. **Preserved**: Provenance section (was already good), chunking, collection choice, metadata conventions, search filters
13. **Preserved**: "Why this skill exists" framing, token efficiency messaging

## What NOT to change

- The frontmatter `name` field stays `ariadne-document-intelligence`
